...
├── scripts/
│   ├── assemble-dist.js  # Copies src/index.html, css/, events/ into dist/ for deployment
...
Events are defined as JSON objects (stored in `src/js/events/phase_*.json`, fetched at runtime). Boss events are marked with the `BOSS: ` title prefix (`CONFIG.game.bossPrefix`); every event may also carry an `emoji` and a short `desc` (used in tooltips and the help screen).
