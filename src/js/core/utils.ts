...
import { CONFIG, STAT_META } from './config.js';
import { RngEngine } from './seeded-rng.js';
import type { StatKey, Stats } from './types.js';

// Dice facade — the single mock seam for tests. The engine calls
// Dice.d20() / Dice.dRoll(n); the test harness swaps the properties to
// control rolls. (ESM function imports are read-only bindings, so a
// mutable object is the seam.) Backed by RngEngine, so seeded runs
// reproduce the same rolls.
export const Dice = {
  dRoll(sides: number): number { return RngEngine.dRoll(sides); },
  d20(): number { return this.dRoll(20); },
};

// Convert in-game days to career years (12 days = 1 career year).
export function dayToCareerYear(day: number): number {
...
export function formatEffects(effects: Partial<Stats>): string {
...
export function equipmentEffectText(item: { effects?: Partial<Stats>; consumableSlots?: number }): string {
  const parts: string[] = [];
...
export function formatSignedEffects(effects: Partial<Stats>): string {
  return Object.entries(effects)
