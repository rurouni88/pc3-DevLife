...
async function initEvents(): Promise<void> {
  // The fetched JSON is untrusted `any` until validateEvents() checks it.
  const promises = Object.values(PHASE_FILES).map(f => fetch(f).then(r => r.json()));
  const results = await Promise.all(promises);
  const events = results.flat() as GameEvent[];
...
function validateEvents(events: GameEvent[]): void {
    ...
    event.choices.forEach((choice, j) => {
      ...
      if (choice.checks) {
        for (const stat of Object.keys(choice.checks)) {
          if (!(CONFIG.stats.keys as string[]).includes(stat)) {
...
function showEventsLoadError(): void {
